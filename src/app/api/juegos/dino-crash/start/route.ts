...
// Abre una ronda de Dino Crash. TODA la lógica (RNG del crash, cobro, seed
// comprometido) vive en la RPC crash_start (SECURITY DEFINER). El punto de crash
// NO se devuelve: queda oculto hasta que la ronda se cierra.
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc("crash_start", { p_bet: bet, p_client_seed: clientSeed });

    if (error) {
      const m = error.message || "";
      if (/saldo_insuficiente/.test(m)) return errorJson("Te quedaste sin fichas para esta apuesta.", 402);
      if (/ronda_activa/.test(m)) return errorJson("Ya tenés una ronda en curso.", 409);
      ...
      return errorJson("No se pudo abrir la ronda: " + m, 500);
    }
