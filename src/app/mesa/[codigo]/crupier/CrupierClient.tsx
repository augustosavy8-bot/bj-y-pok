...
export function CrupierClient({ userId }: { userId: string }) {
  ...
  if (cargando && !mesa) return <Centrado>Cargando…</Centrado>;
  if (!mesa) return <Centrado>No existe la mesa {codigo}.</Centrado>;

  // Blackjack: vista de crupier propia (el guard de crupier ya corrió server-side).
